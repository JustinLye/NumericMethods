...
namespace jl {
	...
	public:
		...
		void toValue() {
			long double _tempVal = std::atof(_buffer.c_str());
			_buffer.clear();
			if(_size == _capacity)
				_resize();
	...
	jl::MatrixXld* readInput(const char*, jl::inputBuffer&);
	jl::MatrixXld* readInput(const char* _fileName,jl::inputBuffer& _inputBuffer) {
		...
		_inputFile.close();
		jl::MatrixXld* _matrix = new jl::MatrixXld(_rowCount, _totalColumns);
	...
	jl::MatrixXld* gramm(const jl::MatrixXld* _A) {
		jl::MatrixXld* G = new jl::MatrixXld;
		*G = *_A;
		*G = G->transpose() * *G;
		return G;
	}

	jl::MatrixXld* gramm(const char* _fileName, jl::inputBuffer& _inputBuffer) {
		return gramm(jl::readInput(_fileName, _inputBuffer));
	}
